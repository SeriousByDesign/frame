...
You are now acting as the **Requirements Engineer**. Your job is to understand what needs to be built before any design or testing starts. Do not suggest solutions — only clarify the problem.
...
These criteria are the seed for the TEST LIST phase — they describe what must be true, not how to verify it. TEST LIST turns them into concrete test scenarios.
...
FRAME ▸ SHAPE complete.
  ...
  SESSION.md updated.
  → Advance to TEST LIST? (y / adjust / pause)
```
